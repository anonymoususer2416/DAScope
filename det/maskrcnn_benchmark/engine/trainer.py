...
from maskrcnn_benchmark.engine.inference import visualize
...
def do_mask_da_train(
    ...
    training_losses = []
    masked_losses = []
    for iteration, ((_, source_images, source_targets, i_), (_, target_images, target_targets, idx2)) in enumerate(zip(source_data_loader, target_data_loader), start_iter):
        model.train()
        model_teacher.eval()
        ...
        record_dict, output = model(images, targets)
        ...
        if len(target_pseudo_labels)>0:
            ...
            masked_loss_dict, output = model(masked_images, masked_target, use_pseudo_labeling_weight=cfg.MODEL.PSEUDO_LABEL_WEIGHT, with_DA_ON=False)
        ...
        if iteration % checkpoint_period == 0 and iteration >= 25000:
            checkpointer.save("model_{:07d}".format(iteration), **arguments)
