...
class FIRoIHead(torch.nn.Module):
    ...
    def forward_train(self,
        ...
        if self.with_bbox:
            ...
            # print("FROI bbox results: " + str(bbox_results))
        ...
        if self.with_mask:
            ...
            # print("FROI mask results: " + str(mask_results))
    ...
    def _update_iq_score_info(self, cat_id, cur_gt_roi_feat):
        ...
        print("saving ROI feat to: " + str(cur_gt_save_pth))
    ...
    def _bbox_forward_train(self, x, assign_results, sampling_results,
        ...
        # print("bbox forward train, loss_bbox: " + str(loss_bbox))
        ...
        for i in range(len(sampling_results)):
            ...
            #     file.write("i: " + str(i) + " num gts: " + str(num_gts) + "\n")
            #     file.write(str(sampling_results[i].pos_inds) + "\n")
            #     file.write(str(sampling_results[i].pos_inds.shape) + "\n")
            #     file.write(str(con_roi_feats.shape) + "\n")
            #     file.write(str(sampling_results[i].pos_inds[:num_gts]) + '\n')
            ind_set = []
            for ind in sampling_results[i].pos_inds[:num_gts]:
                if ind < con_roi_feats.shape[0]:
                    ind_set.append(ind.item())
            anchor_feature = con_roi_feats[ind_set]

            contrast_feature = con_roi_feats[sample_inds[:len(ind_set)]]
            iq_loss_weights = torch.ones_like(iq_scores[:len(ind_set)])

            # print("iq signs: " + str(iq_signs))
            # print(self.iq_loss_weights)
            iq_signs = iq_signs[:len(ind_set)]
            for j, weight in enumerate(self.iq_loss_weights):
                iq_loss_weights[iq_signs == j] *= weight

            pos_signs = pos_signs[:len(ind_set)]
    ...
    def contrast_forward(self, anchor_feature, contrast_feature,
        ...
        # print("anchor feature shape: " + str(anchor_feature.shape))
        # print("contrast feature shape: " + str(contrast_feature.shape))
        # print(pos_signs)
