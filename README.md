# Imitate to Adapt: Domain Adaptive Object Detection via Masked Image Consistency and Imitation Learning

## DAScope Overview

Domain adaptive object detectors aim to classify and locate objects in a scene when domain gaps exist between the training (source) and testing (target) environments. Existing deep learning models for domain adaptive object detection suffer from poor detection performance with small and far-away objects. To address this, we propose DAScope, a novel, small-object-aware domain adaptive object detection pipeline. The key novelty in our proposed pipeline is a new domain adaptive and contrastive feature imitation learning module that extracts improved object-level feature representations, which boosts small object detection. 

<img src="figures/das_overview.png" width="900">

By combining DAScope with an effective baseline detector, Masked Image Consistency (MIC), we achieve competitive performance with the state-of-the-art domain adaptive detectors on benchmark datasets. Shown below are our results on Cityscapes->Foggy Cityscapes. 
...
* [det/maskrcnn_benchmark/modeling/roi_heads/box_head/box_head.py](det/maskrcnn_benchmark/modeling/roi_heads/box_head.py):
